...
interface SaleStoreState {
  ...
  addProductsToCart: (payload: Product[], type?: string) => void;
...
const useSaleStore = create<SaleStoreState>(set => ({
  // initial state
  carts: [],
  maxCart: 3,
  activeCart: null,
  ...
  addProductsToCart: (products: Product[], type = CART_SALES_TYPE.FROM_CART) =>
    set(state => {
